...
async function insertPharmacy(pharmacyData) {
    ...
    const queryText = 'INSERT INTO pharmacy (name, location, phone_number, email, website, opening_hours, profile_picture_url, rating) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *';
  ...
  async function updatePharmacy(pharmacyId, pharmacyData) {
    const { name, location, phone_number, email, website, opening_hours, profile_picture_url, rating } = pharmacyData;
    const queryText = 'UPDATE pharmacy SET name = $1, location = $2, phone_number = $3, email = $4, website = $5, opening_hours = $6, profile_picture_url = $7, rating = $8 WHERE pharmacy_id = $9 RETURNING *';
  ...
  async function getPharmacyById(pharmacyId) {
    const queryText = 'SELECT * FROM pharmacy WHERE pharmacy_id = $1';
    const { rows } = await db.query(queryText, [pharmacyId]);
    return rows[0];
  }
